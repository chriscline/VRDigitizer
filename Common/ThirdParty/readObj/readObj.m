    ...
    switch ln
        ...
        case 'f'   % face definition
           ...
           [tok str] = strtok(str,'//');     % vertex only
			...
			% split quads (or higher) into tris
			if length(fv) > 3
				if length(fv)==4
					newIndices = [1 2 3 1 3 4];
				elseif length(fv)==5
					newIndices = [1 2 3 1 3 4 1 4 5];
				else
					warning('Dropping face with %d vertices',length(fv));
					continue;
				end
				fv = reshape(fv(newIndices),3,[])';
				fvt = reshape(fvt(newIndices),3,[])';
				fvn = reshape(fvn(newIndices),3,[])';
			end
