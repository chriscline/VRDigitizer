		...
		function didPrint = printUpdate(o,isExplicitStart)
			...
			if ~o.isParallel
				if o.doPrintTimeEstimate && timeEstimateValid
					strToPrint = sprintf([o.formatStr '\t Elapsed: %s \t ETR: %s \t ETA: %s'],...
						o.n,o.N,c_relTime_toStr(elapsedTime), ETRStr,ETAStr);
				else
					strToPrint = sprintf(o.formatStr,o.n,o.N);
				end
			else
				if o.doPrintTimeEstimate && timeEstimateValid
					strToPrint = sprintf([o.formatStr ' \t (parallel %d/%d) \t Elapsed: %s \t ETR: %s \t ETA: %s'],...
						o.n,o.N,numFinished+1,o.N,c_relTime_toStr(elapsedTime), ETRStr,ETAStr);
				else
					strToPrint = sprintf([o.formatStr ' \t (parallel %d/%d)'],o.n,o.N,numFinished+1,o.N);
				end
			end
