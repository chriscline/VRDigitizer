...
% c_NetworkInterfacer Wrapper class around various network communication functions
% Can handle TCP/UDP sending/receiving of arbitrary data
%
% Partial example:
%   ni = c_NetworkInterfacer('port',1234,'protocol','TCP');
%   ni.send('Hello, world');
%	pause(1)
%	resp = ni.tryRead('convertTo','char')
%
% Complete examples:
%	UDP: see c_NetworkInterfacer.testFn_judp_tx() and .testFn_judp_rx()
%	TCP: see c_NetworkInterfacer.testFn_tcp_tx() and .testFn_tcp_rx()
%	
			...
			switch(o.method)
				case 'jtcp'
					if s.doBlock
						bytesRead = [];
						numBytes = s.numBytes;
						maxNumBytes = s.maxNumBytes;
						while true
							if o.numBytesAvailable == 0
								pause(0.05);
								continue;
							end
							newBytesRead = o.tryRead(varargin{:},...
								'numBytes',numBytes,...
								'maxNumBytes',maxNumBytes,...
								'doBlock',false,...
								'convertTo','');
							if ~isempty(numBytes)
								numBytes = numBytes - length(newBytesRead);
							end
							if ~isempty(maxNumBytes)
								maxNumBytes = maxNumBytes - length(newBytesRead);
							end
							bytesRead = [bytesRead, newBytesRead];
							
							if ~isempty(numBytes) && numBytes > 0
								continue; % still more to read
							end
							break;
						end
					else
	...
	methods(Static)
		function addDependencies
			...
			if isempty(pathModified)
				...
				addpathIfExists = @(path) c_if(exist(path,'dir')>0,c_void(@()addpath(path),[]),[]);
				
				addpathIfExists(fullfile(mfilepath,'../ThirdParty/judp'));
				addpathIfExists(fullfile(mfilepath,'../ThirdParty/jtcp'));
				addpathIfExists(fullfile(mfilepath,'../ThirdParty/pnet'));
				pathModified = true;
			end
		end
		
		function testFn_tcp_tx()
			port = c_NetworkInterfacer.test_port;
			msg = c_NetworkInterfacer.test_msg;
			
			nitx = c_NetworkInterfacer(...
				'port',port,...
				'protocol','TCP');
			
			c_saySingle('TCP_Tx test waiting to start. Press any key to continue');
			pause
			while true
				nitx.send(msg);
		...
		function testFn_tcp_rx()
			port = c_NetworkInterfacer.test_port;
			nirx = c_NetworkInterfacer(...
				'port',port,...
				'protocol','TCP',...
				'isServer',true,...
				'connectionTimeout',inf);
			
			expectedMsg = c_NetworkInterfacer.test_msg;
			
			c_saySingle('TCP_Rx test starting infinite read loop. Press ctrl-c to terminate.');
			while true
				msg = nirx.tryRead('numBytes',length(expectedMsg),'convertTo','char','doBlock',true);
